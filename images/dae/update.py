...
# v1.1.0 predates the bug, so the fetcher ships the v1.1.0 binary. The image is
# tagged 2.0.1 (not 1.1.0) so flux's highest-semver ImagePolicy rolls forward off
...
def main():
  ...
  match sys.argv[1]:
    ...
    case 'url':
      print(github.get_release_url(repo, BINARY_VERSION, lambda s: 'linux-x86_64_v3_avx2.zip' in s))
    case _:
